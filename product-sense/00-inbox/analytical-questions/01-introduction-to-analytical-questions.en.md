...
## Key Terms
- PM: Product Manager, the person responsible for product strategy, user value, prioritization, and execution.
- A/B test: A controlled experiment that randomly assigns users to a control group and a treatment group to compare product outcomes.
- TAM: Total Addressable Market, the full market opportunity a product could theoretically serve.
- DAU: Daily Active Users, the number of unique users who are active on a product in a day.
- Metrics: Quantitative measures used to evaluate product health, user behavior, business goals, or experiment results.
- North star metric: The single metric that best represents long-term product value and strategic direction.
- GAME framework: A metrics-answer framework: Clarify, Goals, Actions, Metrics, Evaluate.

## Core Review

Analytical interviews are not pure math exercises. They are structured product judgment questions: use data and logic to support a recommendation that makes sense for users, the product, and the business.

A strong answer should move from problem framing to goals, then to data, metrics, tradeoffs, and a clear decision.
