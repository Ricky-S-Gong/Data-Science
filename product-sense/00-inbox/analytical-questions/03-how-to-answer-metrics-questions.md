...
Exponent lesson: How to Answer Metrics Questions

## Abbreviations and Terms

- PM: Product Manager，产品经理。
- KPI: Key Performance Indicator，关键绩效指标。
- North star metric: 北极星指标，最能概括产品或业务成功的单一核心指标。
- GAME framework: 回答指标题的五步框架，分别是 Clarify, Goals, Actions, Metrics, Evaluate。
- DAU: Daily Active Users，日活跃用户数。
- CTR: Click-through Rate，点击率，通常为 clicks / impressions。
- OKR: Objectives and Key Results，目标与关键结果。
- MTTR: Mean Time To Resolution，平均解决时间。
- Churn: 流失，通常指用户停止使用产品或取消订阅。
- Customer lifetime value: 客户生命周期价值，常缩写为 CLV 或 LTV。
- Guardrail metrics: 护栏指标，用来确保优化某个目标时没有伤害其他重要体验或业务指标。
- Counter metrics: 反向/制衡指标，和 guardrail metrics 类似，用来观察目标指标提升是否带来副作用。
- User journey map: 用户旅程图，描述用户为完成某个目标经过的步骤、页面、决策点和行为。
- User funnel: 用户漏斗，描述用户从进入产品到完成目标行为的流程及每一步转化。

## Image Assets

Use the original images provided with this lesson for the future website.

Expected paths:

- `product-sense/assets/images/analytical-questions/metrics/game-framework.png`
- `product-sense/assets/images/analytical-questions/metrics/whiteboard-1-clarify.png`
- `product-sense/assets/images/analytical-questions/metrics/whiteboard-2-goals.png`
- `product-sense/assets/images/analytical-questions/metrics/whiteboard-3-actions.png`
...
Goal -> User actions -> Metrics -> Evaluation
...
中文：你会如何衡量 Instagram Discover feed 的成功？
...
User journey map 或 user funnel 是一个流程图，覆盖用户为了完成某个目标会经历的每一步。完整的软件产品 walkthrough 会包括用户看到的每个 screen、每个 decision point 和每一次 click。

构建 user journey map 的好处是，它会让你的选项显性化。建好 journey map 后，你可以选择那些真正推动目标的 actions，并在后面的回答中选择指标来衡量这些 actions。

构建 user journey map 有两个关键步骤：

1. 选择一个你想分析的 product use case。

选择一个用户目标与产品目标一致的 use case。面试中常见错误是为了省时间一次性 map 多个 use cases，这很容易让你在不同用户行为之间混乱。如果有必要，应该分别走多个 user journeys。

2. 走一遍用户为了达成目标会采取的步骤。

选定清晰 use case 后，列出用户与产品互动时经历的步骤。从用户进入产品的位置开始，列出每个 screen、decision point 和 click。记得定义一个清晰的 endpoint。这个过程可能花时间，所以你要用 PM 判断力略过和当前问题不重要的细节。

在推进 journey 的过程中，记录那些支持前面目标的 essential steps 或 desired user behaviors。下一步你很可能会为这些 actions 生成 metrics。
...
- Product metrics：捕捉用户如何与产品互动，帮助理解用户行为和做产品决策。常见例子包括 DAU、CTR、Time Spent。
- OKRs：把战略目标和可量化 key results 连接起来，帮助团队对齐并追踪进度。OKRs 对战略很重要，但范围有意限制，不一定能反映整体业务健康或增长。PM 的 north star 往往会和 OKR 有关。
- Business metrics：衡量公司整体表现的高层指标。它们通常最接近 business health 的 true north，但往往是 lagging indicators，小功能很难显著影响它们，所以对战术决策不一定有用。例子包括 revenue、net profit margin、customer lifetime value。
- Quality metrics：衡量流程或产品运行质量，可以是内部或外部质量。例子包括 bug backlog size、MTTR、page load time。
- Leading indicators：支持实时决策，通常更敏感、更容易波动。很多 product metrics 也可以作为 leading indicators。为了形成完整理解，leading indicators 应和更稳固的 lagging indicators 一起使用。例子包括 user signups、button clicks。
- Counter 或 guardrail metrics：当推动一个指标可能伤害另一个重要领域时，用 counter metrics 确保追求某个目标不会牺牲其他优先级。例子包括 churn rate、customer satisfaction scores。
...
第五步是 Evaluate。所有指标都有强项和弱点。评估指标选择时，要描述你考虑过的不同指标之间的 tradeoffs。

最重要的是判断：你选择的指标是否足够好，能否衡量支持目标的关键行为。你可以问自己：“我的指标在哪些地方会失效？”

面试官也经常在追问中深入：既然你选的指标有天然弱点，你会如何处理？

并不是所有弱点都必须解决。有些弱点只是 PM 需要意识到的背景风险。如果某个问题值得缓解，你就要告诉面试官你会怎么做。很多时候，设置 counter metrics 能帮助你确认用户体验没有在其他地方变差。
...
选择 time spent 有道理，因为它能体现用户是否愿意深入消费内容。但它也可能误导，因为用户停留很久可能是因为没找到感兴趣内容。更稳妥的做法，是把 time spent 和 likes、followed creators 等 engagement metrics 结合起来。
...
Good answer：能基于合理的产品或战略洞察，清楚说明产品目标。候选人能解释哪些 actions 和 behaviors 可以证明这些目标正在达成，然后找到衡量这些 actions 的 metrics。North star metric 和目标之间联系清楚。候选人也理解自己的指标可能在哪些地方误导，并能提出 guardrails 和 alternate metrics。

Great answer：体现出对产品及其存在理由的深刻理解，可能涉及用户、产品如何支持公司战略目标、市场位置等。在此基础上，候选人能提出具体目标，清楚表达这个产品最重要的价值。候选人知道自己需要理解产品的哪些方面，并能把这些理解转化为 actions，再转化为 metrics。候选人能自如地思考如何把用户行为转化成指标，以及捕捉这些指标意味着什么。North star metric 清晰，理由和产品目标、甚至组织层面的成功深度绑定。
